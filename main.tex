...
\begin{description}[leftmargin=3.4cm,style=nextline]
  ...
  \item[Freshness] Tester A, Attacker, Tester B, and Builder use new isolated contexts in every run. Each worker locks its first routing key and accepts only same-key corrections.
  ...
  \item[Terminal broker check] After the final response, the reference harness requires Store approval-ready state, an even request/response session ending in an assistant response, and at least 60 seconds of same-key no-job polling.
  ...
  \item[Streak reset] The consecutive-run count resets when a run cannot converge, loses its job or routing identity, violates the security matrix, or terminates with required evidence still failing, pending, lost, or unverifiable. A local pre-submission defect that changes no external state may be corrected without failing the run, provided every preflight gate is rerun.
\end{description}
